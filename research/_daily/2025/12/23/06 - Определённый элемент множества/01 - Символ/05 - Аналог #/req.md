...
# 2. `G↑`
Теперь я хочу добавить к `ᗉ` новый символ (`O`), чтобы совместная запись `ᗉ` с `O` означала «конкретный, определённый элемент `S`» (аналог артикля «the» в английском языке).
...
##
`O` должен быть визуально лаконичным: ведь он будет использоваться совместно с `ᗉ`.
##
Я хочу, чтобы `O` был визуально похож на `#`.
Мотивация:
``` 
Символ решётки (хеш) стандартно используется для обозначения уникального идентификатора (ID) объекта.
```
При этом `O` должен быть редким, уникальным, не стандартным `#`.
##

# `᛭T`
##
Перечисли мне `AA`: 20 символов Unicode (помимо латинской буквы `P`), которыми принято обозначать предикат: https://en.wikipedia.org/wiki/Predicate_(logic)
##
`AA` должны быть буквами, а не математическими операторами типа `=`.
##
`AA` должны быть по возможности уникальными: чтобы по букве было понятно, что она обозначает предикат.

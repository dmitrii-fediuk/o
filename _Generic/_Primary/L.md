...
~~~
## Meaning
Такая запись вводит понятие `A` как «определённый элемент `S`».
###
`S` — это `𝕊` / `𝒞` / `ℂ` / `𝕋`.
###
Cинтаксис `A ⦿ S` не содержит конкретики, какой именно элемент `A` имеется в виду.
Эта конкретика будет задана в дальнейшем, отдельными выражениями.
###
По смыслу `⦿` соответствует определённому артиклю «the» английского языка.
## Example
`ꆜ ⦿ ⠿ ⟨ клиент ⟩` означает, что `ꆜ` — это не неопределённый клиент, а некий конкретный клиент.
Конкретная информация о `ꆜ` будет задана в дальнейшем, отдельными выражениями.
## Следствие
⊤ ⟨ `A` ∈ `S` ⟩ 

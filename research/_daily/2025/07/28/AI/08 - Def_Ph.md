...
`K` ≔
```markdown
Ключевое слово `Def_Ph` в контексте пункта 5 выше. 
```
...
`≔` в этом двухсимвольном обозначении означает «definition» (смотри пункт 1 выше), а второй символ будет означать «surprising».

#
`A` ≔ ⟨ https://en.wikipedia.org/wiki/Actor_(UML) ⟩ 

#
`S` ≔ ⟨ символ Unicode, внешне похожий на `A` ⟩ 

# Примеры `S` (их повторно не предлагай)
`ꆜ` `ꖵ`
...
##
`S` должен быть одним символом Unicode.

##
Для каждого `S` укажи его семантику.

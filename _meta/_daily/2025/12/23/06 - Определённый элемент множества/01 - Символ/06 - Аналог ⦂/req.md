...
##
Я хочу, чтобы `O` выглядел как нестандартное двоеточие: типа `⦂`.
При этом сам `⦂` в качестве `O` мне не нравится своей громоздкостью (слишком большие точки).
Но и слишком маленькими точки тоже не должны быть.
##

#
`U` ≔ `https://en.wikipedia.org/wiki/Ontology_(information_science)`

#
`P` ≔ `Ontology_(information_science)`

# `᛭T`
##
Как по-правильному на английском языке называется заключительная часть пути URL (например, `P` относительно `U`)?
##
Предложи 5 лучших вариантов и объясни разницу между ними.

...
# 2. `G↑`
Мне нужны `QQ`: пары необычных символов, зеркальных относительно друг друга, которые можно использовать как парные кавычки.
## Пример:
`ᐸ` и `ᐳ`.
Конкретно этот вариант мне не очень нравится: эти символы слишком широкие.
Это просто пример.

# `᛭T`
##
Предложи мне 50 вариантов `QQ`.
##
Каждый из пары должен быть одним символом Unicode.
##
Не предлагай громоздкие варианты типа `༼` `༽`.
## 
...
## 
Не предлагай `QQ` из шаблонных мест типа таблиц спецсимволов.
Вместо этого ищи `QQ` в редких алфавитах или среди редких иероглифов.
Особенно там, где на самом деле `QQ` — это не кавычки, а лишь пары зеркальных символов, которые могли ьы использоваться как кавычки.

...
`K` ≔
```markdown
Ключевое слово `Def_Set` в контексте пункта 7 выше. 
```
...
`≔` в этом двухсимвольном обозначении означает «definition» (смотри пункт 1 выше), а второй символ будет означать «множество».

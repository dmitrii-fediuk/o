...
`K` ≔
```markdown
Ключевое слово `Def_F` в контексте пунктов 4.2 и 4.3 выше. 
```
...
1) `KAi` в контексте пункта 4.3 должен состоять из 2 символов: `≔` и ещё одного символа Unicode (до или после `≔`).
`≔` в этом двухсимвольном обозначении означает «definition» (смотри пункт 1 выше), а второй символ будет означать «факт».

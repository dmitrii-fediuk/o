...
`K` ≔
```markdown
Ключевое слово `Def_P`, которое я использую в пункте 1.2 выше.
```
...
`≔` в этом двухсимвольном обозначении означает «definition» (смотри пункт 1 выше), а второй символ будет означать «проблема».

...
#
По сути, `<некий символ>` означает: «следующие дальше attributes / properties относятся к `A`, являются частью `A`».

#
Потенциально я буду употреблять `T` несколько раз для одного и того же `A`.
Это значит, что в качестве `<некий символ>` не подходят символы эквивалентности.
Каждое употребление `T` описывает для `A` некие новые (не описанные ранее) свойства.
...
##
Рекомендую для поиска обратиться к математике, логике, редким алфавитам.

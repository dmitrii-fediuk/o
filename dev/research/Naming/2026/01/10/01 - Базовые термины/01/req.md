#
`ꙮL` ≔ ⟨ прикладной язык для описания онтологии и задачи, которую LLM надлежит решить ⟩  
#
`G` ≔ ⟨ Глоссарий терминов, используемых `ꙮL` ⟩ 
#
`S⠿` ≔ ⠿ ⟨ Разделы `G` ⟩ 
#
`B` ≔ ⦿ ⟨ Базовый раздел `S⠿` ⟩
#
`B` содержит базовые термины.
#
`A⠿` ≔ ⠿ ⟨ Варианты имени для `B` ⟩  
#
`Aᵢ` ᗉ `A⠿`
# `᛭T`
Придумай 40 `Aᵢ`.
Каждый `Aᵢ` обоснуй.
#
`Aᵢ` должны быть на английском языке.
#
`Aᵢ` должно быть одним словом.
# Варианты `Aᵢ`, которые пока мне нравятся больше всего
⟪ Их не повторяй ⟫
- Primary
- Basic
- Basis
- General
- Core
# Варианты `Aᵢ`, которые мне не нравятся
⟪ Их не повторяй ⟫
⟪ Пока отсутствуют ⟫

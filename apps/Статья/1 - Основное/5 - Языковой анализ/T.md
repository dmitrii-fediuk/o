...
# Не предлагай следующие правки
##
Не предлагай замену «letter» на «email».
...
Не придирайся к следующему:
```
В тексте использование слов «formulate» и «formulating» звучит излишне академично для описания подготовки коммерческого предложения.
```

##
Не придирайся к следующему:
```
Вводное слово «So» в начале предложения задаёт излишне разговорный тон.
```

##
Не придирайся к следующему:
```
Фраза «a lot of» является слишком разговорной для деловой переписки, поэтому её лучше заменить на «a significant number of».
```

##
Не придирайся к следующему:
```
Использование символа `§` в деловом предложении на английском языке выглядит излишне формально
```

##
Не придирайся к следующему:
```
В англоязычной типографике длинное тире обычно не отделяется пробелами от соседних слов.
```

##
Не придирайся к следующему:
```
Выражение «works incorrectly» звучит недостаточно профессионально для деловой коммуникации
```

##
Не придирайся к следующему:
```
Использование математического символа `≥` в тексте на английском языке не соответствует стандартам делового стиля.
```
# `᛭T`
##
Перечисли мне `AA`: 50 символов Unicode, которыми можно кратко обозначить понятие «predicate» из https://en.wikipedia.org/wiki/Predicate_(logic) / https://en.wikipedia.org/wiki/First-order_logic / https://en.wikipedia.org/wiki/Function_symbol
##
`AA` должны быть буквами, а не математическими операторами.
##
`AA` должны быть по возможности уникальными: чтобы по букве было понятно, что она обозначает.
Аналогично, например, уникальной букве `ℝ` для понятия «множество вещественных чисел». 
По этой причине не предлагай мне стандартные буквы английского алфавита.
Однако можно предлагать их вариации с нестандартным написанием (как `ℝ`).
##
Можно предлагать буквы редких алфавитов, визуально похожих на `P` или `П`. 
##
Для каждого символа укажи его семантику.

...
##
Я хочу, чтобы `O` выглядел похоже на `⦿`, `⊙`, `ʘ`.
То есть что-то типа круга с точкой (или минималистичным символом) внутри.
Уже перечисленные мной примеры не предлагай.
##
`O` должен быть одним символом Unicode.
Не предлагай мне просто дописать точку отдельным символом.
##
Для каждого `O` укажи традиционную семантику.

##
Предлагай только редкие, уникальные `O`.
Не предлагай широко распространённые типа простых геометрических фигур.

##
Не предлагай математические символы.
Мне нужны только буквы, иероглифы и т.п.

##
Поищи `O` в редких алфавитах. 

##
Помимо редких алфавитов, поищи `O` среди редких иероглифов. 

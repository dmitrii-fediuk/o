...
Def(`K`):
```
Ключевое слово `Def`, которое я использую в `R1`.
```
...
1) `KAi` должен быть короче `K`.
2) Вероятно, для лаконичности, стоит подумать об использовании символа Unicode или общепринятой для подобной цели греческой буквы.
